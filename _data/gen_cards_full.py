...
D = json.load(open('_data/fia_data.json', encoding='utf-8'))
...
UI = {
 ...
 "doc":  S("Dokument öffnen","Open document","Otvoriť dokument","Otvori dokument","Otwórz dokument","Abrir documento","Apri documento","Ouvrir le document","Öppna handling"),
...
.item{background:#fff;border-radius:7px;box-shadow:0 1px 6px rgba(31,56,100,.09);padding:12px 15px;margin-bottom:9px}.item.imp{border-left:4px solid var(--red)}
.itop{display:flex;justify-content:space-between;gap:10px;margin-bottom:4px}.thr{display:inline-flex;align-items:center;justify-content:center;min-width:20px;height:20px;padding:0 6px;border-radius:11px;background:var(--navy);color:#fff;font-size:12px;font-weight:800;margin-left:auto}.cbtns{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}.cb{font-size:13px;border:1px solid var(--bd);background:#fff;border-radius:6px;padding:6px 14px;cursor:pointer;color:#3d4d5e}.cb:hover{border-color:var(--navy);color:var(--navy)}.begbox{display:none;margin-top:9px;padding:11px 13px;background:#f7fafd;border:1px solid var(--bd);border-radius:6px;font-size:13.5px;line-height:1.6}.begbox.on{display:block}.begbox p{margin:0 0 9px}.sharewrap{position:relative;display:inline-block}.sharebox{display:none;position:absolute;bottom:calc(100% + 6px);left:0;background:#fff;border:1px solid #ddd;border-radius:10px;box-shadow:0 6px 20px rgba(0,0,0,.15);padding:6px;flex-direction:column;min-width:170px;z-index:50}.sharebox.on{display:flex}.sharebox a{padding:8px 10px;border-radius:6px;color:#222;text-decoration:none;font-size:14px;white-space:nowrap}.sharebox a:hover{background:#f2f4f8}
...
.acc{font-size:11px;border-radius:999px;padding:1px 9px;white-space:nowrap}.acc-pub{background:#e9f7ef;color:#0B7A3B;border:1px solid #b7e2c8}.acc-pw{background:#fff3d6;color:#8a5a00;border:1px solid #f0c040}
.iorg{font-size:12.5px;color:#6B7C91;margin:4px 0}.fn{font-size:11.5px;color:#6B7C91;font-style:italic;background:#fffdf3;border:1px dashed #f0c040;border-radius:4px;padding:3px 8px;margin:5px 0}.kom{background:#eef4ff!important;border-color:#c3d5f5!important;color:#1F3864}.cmt-wrap{margin-top:9px}.cmt-wrap:empty{display:none}
...
# ─────────── KARTY KONANI ───────────
def item_html(p, n):
    import urllib.parse as _u, hashlib as _h
    url = p.get("urls") or {}
    doc = "".join(f'<span class="gtl {L}"><a class="b" href="{html.escape(url.get(L,""))}" target="_blank" rel="noopener">📄 {UI["doc"][L]}</a></span>' for L in LANGS if url.get(L))
    og = ""
    if p.get("orig"):
        viewer = "https://peterferenc246-design.github.io/fia/viewer.html?doc=" + _u.quote(p["orig"], safe="")
        og = f'<a class="b b-qes" href="{html.escape(viewer)}" target="_blank" rel="noopener">✍ {g(UI["orig"])}</a>'
    sm = ""
    sid = p.get("summodId", "")
    if sid and sid in SUHRNY:
        s = SUHRNY[sid]
        blocks = "".join(f'<div class="gtl-b {L}">{s.get(L, "")}</div>' for L in LANGS if s.get(L))
        sm = f'<button class="b" onclick="document.getElementById(\'s{n}\').classList.toggle(\'on\')">📄 {g(UI["sum"])}</button><div class="sumbox" id="s{n}">{blocks}</div>'
    subj = p.get("subj9") or {}
    subj_sk = subj.get("sk") or next((v for v in subj.values() if v), "")
    cid = "fia-" + _h.sha1((str(p.get("karta", "")) + "|" + str(p.get("date", "")) + "|" + subj_sk).encode("utf-8")).hexdigest()[:12]
    kom = f'<button class="b kom" onclick="toggleCmt(this)">💬 {g(UI["kom"])}</button>' if p.get("comments") or p.get("komentare") or p.get("cmtid") else ""
    fallback = p.get("fallback") or ""
    if not doc and fallback:
        doc = f'<span class="gtl sk"><a class="b" href="{html.escape(fallback)}" target="_blank" rel="noopener">📄 {g(UI["doc"])}</a></span>'
    acc = "pw" if p.get("access") == "pw" else "pub"
    court = html.escape(p.get("court", "") or "")
    thread = html.escape(str(p.get("thread", "") or ""))
    thrno = html.escape(str(p.get("thrNo", "") or ""))
    thr = f'<span class="thr" data-thread="{thread}" data-thrno="{thrno}"></span>' if thread else ""
    important = " imp" if p.get("dolezite") or p.get("important") else ""
    return (f'<div class="item{important}" data-cid="{cid}" data-ctitle="{html.escape(subj_sk)}" data-thread="{thread}" data-thrno="{thrno}"><div class="itop"><span class="idate">{html.escape(p.get("date", ""))}</span>'
            f'<span class="acc acc-{acc}">{"🔒" if acc == "pw" else "🌐"} {"Password" if acc == "pw" else "Public"}</span>{thr}</div>'
            f'<div class="isubj">{"❗ " if important else ""}{g(subj)}</div>'
            + (f'<div class="iorg">⚖ <b>Orgán:</b> {court}</div>' if court else "")
...
(function(){var groups={};document.querySelectorAll('.item[data-thread]').forEach(function(el){var t=el.dataset.thread;if(!t)return;(groups[t]=groups[t]||[]).push(el);});Object.keys(groups).forEach(function(t){groups[t].forEach(function(el,i){var b=el.querySelector('.thr');if(b)b.textContent=el.dataset.thrno||String(i+1);});});})();
function toggleCmt(btn){
 var it=btn.closest('.item'), box=it&&it.querySelector('.cmt-wrap'); if(!box)return;
 if(box.dataset.open==='1'){box.innerHTML='';box.dataset.open='0';return;}
 box.innerHTML=''; var host=document.createElement('div'); host.dataset.host='https://cusdis.com'; host.dataset.appId=CUS_APP; host.dataset.pageId=it.dataset.cid; host.dataset.pageUrl=location.href; host.dataset.pageTitle=it.dataset.ctitle||document.title; host.dataset.theme='light'; box.appendChild(host); box.dataset.open='1';
 if(!document.querySelector('script[data-cusdis-loader]')){var s=document.createElement('script');s.async=true;s.defer=true;s.src='https://cusdis.com/js/cusdis.es.js';s.dataset.cusdisLoader='1';document.head.appendChild(s);}
 (function wait(){if(window.CUSDIS&&window.CUSDIS.renderTo){window.CUSDIS.renderTo(host);}else{setTimeout(wait,150);}})();
}
"""
...
for cid, ps in pol_karty.items():
    ...
    card_title = (k.get("nazov") or {}).get("sk") or cid
    spr = k.get("sprievodny") or {}
    beg = f'<button class="cb" onclick="document.getElementById(\'b-{cid}\').classList.toggle(\'on\')">📝 {g(UI["beg"])}</button>' if any(spr.values()) else ""
    begbox = f'<div class="begbox" id="b-{cid}">{g(spr, True)}</div>' if any(spr.values()) else ""
    body = f"""
<div class="navbar"><a class="hm" href="register.html">{g(UI["back"])}</a><div class="lang"><span class="lb">LANG</span>{FLAGS}</div></div>
<div class="chead"><h2>{g(k.get("nazov")) or html.escape(cid)}</h2><div class="org">{g(k.get("organ"))}</div>
<div class="meta"><b>{html.escape(k.get("az", "") or "—")}</b> · {html.escape(k.get("ourref", "") or "")} · {html.escape(k.get("datum_txt", "") or k.get("date", ""))} · {len(ps)} {g(UI["docs"])}</div></div>
<div class="cols"><div class="col"><h3>▲ {g(UI["sent"])} ({len(sent)})</h3>{hs}</div><div class="col"><h3>▼ {g(UI["recv"])} ({len(recv)})</h3>{hr}</div></div>
<div class="cbtns">{beg}{share_dropdown(cid, card_title)}</div>{begbox}"""
    open(f'karta-{cid}.html', 'w', encoding='utf-8').write(page(k.get("nazov", {}).get("sk", cid), body, CARD_SCRIPT))
