...
REGISTER_URL = "https://aac327c2.foxprof-koncept.pages.dev/register.html"
...
def polozka(p, n):
    ...
    return (f'<div class="it{" imp" if p.get("dolezite") else ""}"{thr}>'
            ...
            + fn +
            f'<div class="ibt">{doc}{og}{sm}{kom}</div>{subox}<div class="cmt-wrap"></div></div>')
...
.cl{flex:1}
.ct{font-size:16px;font-weight:700;color:var(--navy);line-height:1.35;text-align:justify}
.cdate{font-size:12.5px;color:var(--muted);font-weight:600;margin-right:8px;white-space:nowrap}
.csub{font-size:13.5px;font-weight:600;color:var(--navy);margin:3px 0;text-align:justify}
.cmeta{font-size:12.5px;color:var(--muted);margin-top:3px;text-align:justify}
...
.isj{font-size:14px;font-weight:600;color:var(--navy);margin:4px 0 3px;line-height:1.35;text-align:justify}
.iorg{font-size:12.5px;color:var(--muted);text-align:justify}
